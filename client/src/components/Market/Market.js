...
const Market = () => {
  ...
  const allData = useFetchData(); // Call the custom hook to fetch data

  // State to hold filtered data based on pincode
  const [filteredData, setFilteredData] = useState([]);
  const [inputPincode, setInputPincode] = useState(""); // State to hold input pincode

  useEffect(() => {
    if (allData) {
      setLoading(false); // Set loading to false if data is fetched
    }
  }, [allData]);
